...
public:
    ...
    std::vector<std::string> SupportedSoundFileTypesAsVector() const
    {
        ...
        std::istringstream f(SupportedSoundFileTypes);
        std::string s;
        while (getline(f, s, ' ')) {
            vec.push_back(s);
        }
    ...
    AnimationSounds *GetAnimationSounds(int animationID);
...
protected:
    bool AddOrUpdateAnimationState(int animationID, const char *pPropertyName, const char *pValue);
