...
ISoundEngine *XRSoundEngine::s_pSoundEngine = nullptr;
...
bool XRSoundEngine::InitializeSoundEngine()
{
    ...
    // sound (was) initialized successfully!
    return true;
}

// Perform one-time static cleanup of our singleton.
void XRSoundEngine::DestroySoundEngine()
{
    ...
    sprintf(logMsg, "%s terminating.", GetVersionStr());
...
bool XRSoundEngine::LoadWav(const int soundID, const char *pSoundFilename, const XRSound::PlaybackType playbackType)
{
    if (!IsSoundEngineInitialized())
        return false;
    if (!pSoundFilename || !*pSoundFilename || !strcmp(pSoundFilename,"none"))
    {
    ...
    if (pContext)
    {
        ...
        if (!strcasecmp(pContext->csSoundFilename.c_str(),pSoundFilename))
    ...
    s_pSoundEngine->preload(pSoundFilename);
...
bool XRSoundEngine::PlayWav(const int soundID, const bool bLoop, const float volume)
{
    ...
    if (pISound == nullptr)
    {
        ...
        pISound = pContext->pISound = s_pSoundEngine->play2D(pContext->csSoundFilename.c_str(), bLoop, true, true);
...
// Stop playing the wav file with the specified ID and free its resources (ISound interface).
//   soundID: unique sound ID originally passed to LoadWav
//
...
bool XRSoundEngine::StopWavImpl(WavContext *pContext, XRSoundEngine *pEngine)
{
    ...
    if (pISound)   // was sound ever started via PlayWav?
    {
        ...
        pISound->drop();    // free resources for this sound
...
void XRSoundEngine::WriteLog(const char *pMsg)
{
    ...
    char cbuf[512];
    sprintf(cbuf, "[%s][%.03lf] %s", GetLogID(), XRSoundDLL::GetAbsoluteSimTime(), pMsg);
    ...
    if ((m_lastLogLine != cbuf) /* || (simt >= m_nextDuplicateLogLineSimt) */)
    {
        m_lastLogLine = cbuf;
        // m_nextDuplicateLogLineSimt = simt + 1.0;
        m_pConfig->WriteLog(cbuf);
    }
}
...
const char *XRSoundEngine::GetVersionStr()
{
    if (s_csVersion.empty()) {
        char cbuf[256];
        sprintf(cbuf, "XRSound %.2f %s[%s %s], Build Date : %s", XRSOUND_ENGINE_VERSION, XRSOUND_BETA_STR, ARCH_TYPE, BUILD_TYPE, __DATE__);
        s_csVersion = cbuf;
    }
...
bool XRSoundEngine::PauseOrReumseMusic(const bool bPause)
{
    ...
    if (s_pMusicFolderWavContext && !s_pMusicFolderWavContext->bPaused)  
    {
        ...
        if (pISound)
        {
            ...
            if (bPause)
            {
                sprintf(cbuf, "Pausing global music slot %s", s_pMusicFolderWavContext->ToStr().c_str());
                pISound->setIsPaused(true);
            }
            else
            {
                sprintf(cbuf, "Unpausing global music slot %s", s_pMusicFolderWavContext->ToStr().c_str());
                pISound->setIsPaused(false);
            }
            XRSoundDLL::WriteLog(cbuf);
        }
    }
