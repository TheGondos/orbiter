...
bool XRSoundImpl::Initialize(VESSEL *pVessel)
{
    ...
    m_pEngine = GetXRSoundEngineInstance(pVessel->GetHandle());   // returns nullptr if sound initialization fails
...
bool XRSoundImpl::Initialize(const char *pUniqueModuleName)
{
    ...
    m_pEngine = GetModuleXRSoundEngineInstance(pUniqueModuleName);   // returns nullptr if sound initialization fails or if another module has previously registered using pUniqueModuleName
