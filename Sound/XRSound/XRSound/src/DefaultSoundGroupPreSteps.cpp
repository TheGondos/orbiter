...
bool DefaultSoundGroupPreStep::SetFolder(const char *pFolderSubpath)
{
    ...
    {
        char cbuf[256];
        sprintf(cbuf,"DefaultSoundGroupPreStep::SetFolder ERROR: folder path '%s' set for soundID %d does not exist.",
    ...
    {
        char cbuf[256];
        sprintf(cbuf,"DefaultSoundGroupPreStep::SetFolder ERROR: folder path '%s' set for soundID %d exists, but no supported sound file types found.",
            pFolderSubpath, m_soundID);
        WriteLog(cbuf);
        return false;
    }
    ...
    VERBOSE_LOG(m_pEngine, "DefaultSoundGroupPreStep::SetFolder: successfully scanned folder path '%s' for soundID %d: %d sound file(s) found with extensions [%s].",
        pFolderSubpath, m_soundID, m_pSoundFilesList->GetScannedFileCount(), static_cast<const char *>(GetConfig().SupportedSoundFileTypes.c_str()));
    m_bWavPresent = true;
...
std::string DefaultSoundGroupPreStep::GetRandomSoundFile()
{
    ...
    {
        char cbuf[256];
        sprintf(cbuf, "DefaultSoundGroupPreStep::GetRandomSoundFile WARNING: sound files list is empty for soundID %d", m_soundID);
        WriteLog(cbuf);
    }
    return file;
}
...
std::string DefaultSoundGroupPreStep::GetNextSoundFile()
{
    ...
    {
        char cbuf[256];
        sprintf(cbuf, "DefaultSoundGroupPreStep::GetNextSoundFile WARNING: sound files list is empty for soundID %d", m_soundID);
        WriteLog(cbuf);
    }
    return file;
}
...
void RandomDefaultSoundGroupPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    ...
    {
        ResetTimer();
        //double ut = XRSoundDLL::GetSystemUptime();
        //assert(ut > simt);
            ...
            else  // normal altitude checks
            {
                ...
                if (altitude <= altitudeCallouts[0])
                {
                    for (int i = 0; i < (sizeof(altitudeCallouts) / sizeof(double)); i++)
                    {
                        ...
                        {
                            char cbuf[256];
                            sprintf(cbuf, "%d", static_cast<int>(a));
                            if (LoadAndPlayWavWithBasename(cbuf))
                                SetNextMinimumCalloutTime(simt);
                            break;
                        }
                    }
...
void DockingCalloutsDefaultSoundGroupPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    ...
    else  // not docked
    {
        ...
        {
            // NOTE: we do not just use LoadAndPlayWav here because we need to load this into the correct slot in case the user disabled it via code!
            LoadAndPlayWavUsingID(XRSound::Undocking, GetConfig().Undocking.c_str(), false, XRSound::PlaybackType::InternalOnly);
        ...
        if ((simt >= m_nextMinimumCalloutTime) && (m_previousFrameDistance >= 0))
        {
            ...
            if (distance <= distanceCallouts[0])
            {
                for (int i = 0; i < (sizeof(distanceCallouts) / sizeof(double)); i++)
                {
                    ...
                    {
                        char cbuf[256];
                        sprintf(cbuf, "%d", static_cast<int>(dist));
                        if (LoadAndPlayWavWithBasename(cbuf))
                            SetNextMinimumCalloutTime(simt);
                        break;
                    }
...
void MachCalloutsDefaultSoundGroupPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    ...
    {
        m_previousMach = (double)LONG_MAX;   // out of the atmosphere
    ...
    if ((simt >= m_nextMinimumCalloutTime) && (m_previousMach >= 0))
    {
        ...
        else  // perform standard mach callouts
        {
            for (double m = 2; m < 27; m++)
            {
                ...
                {
                    char cbuf[256];
                    sprintf(cbuf, "Mach %d", static_cast<int>(m));
                    LoadAndPlayWavWithBasename(cbuf);
                    break;
                }
            }
        }
...
void MusicDefaultSoundGroupPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    ...
    // To prevent intermittent music stuttering on Orbiter startup when Orbiter hangs for a full second, which starves the engine, 
