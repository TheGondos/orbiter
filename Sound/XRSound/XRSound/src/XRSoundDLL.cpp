...
VesselXRSoundEngine *XRSoundDLL::GetXRSoundEngineInstance(const OBJHANDLE hVessel, const bool bInvokedByClientVessel)
{
    ...
    if (!bIsValidVessel)
    {
        // should never happen!
        char cbuf[256];
        sprintf(cbuf, "XRSoundDLL::GetXRSoundEngineInstance ERROR: vessel handle passed is invalid! bInvokedByClientVessel = %d", bInvokedByClientVessel);
        WriteLog(cbuf);
        return nullptr;
    }
    ...
    if (!pEngine)   // no existing engine found for this vessel
    {
        ...
        const int dwThrusterCount = pVessel->GetThrusterCount();
        const bool bShouldHaveDefaultSounds = (dwThrusterCount > 0);
        char cbuf[256];
        sprintf(cbuf, "'%s' [class name '%s'], bInvokedByClientVessel = %d, dwThrusterCount = %u, bShouldHaveDefaultSounds = %d",
            pVessel->GetName(), pVessel->GetClassName(), bInvokedByClientVessel, dwThrusterCount, bShouldHaveDefaultSounds);
        std::string csVesselDesc = cbuf;
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "XRSoundDLL::GetXRSoundEngineInstance: creating new XRSoundEngine instance for vessel %s", csVesselDesc.c_str());
...
ModuleXRSoundEngine *XRSoundDLL::GetXRSoundEngineInstance(const char *pUniqueModuleName)
{
    ...
    if (!bIsValidModuleID)
    {
        // should never happen!
        char cbuf[256];
        sprintf(cbuf, "XRSoundDLL::GetXRSoundEngineInstance ERROR: pUniqueModuleName passed is nullptr or empty!");
        WriteLog(cbuf);
        return nullptr;
    }
    ...
    {
        char cbuf[256];
        sprintf(cbuf, "XRSoundDLL::GetXRSoundEngineInstance: creating new XRSoundEngine instance for module '%s'", pUniqueModuleName);
        ...
        {
            sprintf(cbuf, "XRSoundDLL::GetXRSoundEngineInstance: WARNING: nullptr or empty module name supplied; returning nullptr.");
        }
        s_pInstance->WriteLog(cbuf);
    }
...
void XRSoundDLL::ParseGlobalConfigFile()
{
    ...
    {
        char cbuf[256];
        sprintf(cbuf, "Error parsing configuration file '%s' -- see above error messages for details.", globalConfig.GetDefaultFilename());
        globalConfig.WriteLog(cbuf);
    }
}
...
void XRSoundDLL::UpdateAllVesselsMap()
{
    ...
    for (unsigned int i = 0; i < deletedVesselHandles.size(); i++)
    {
        ...
        if (it != m_allVesselsMap.end())
        {
            ...
            XRSoundEngine::DestroyInstance(pEngine);    // this also stops all sounds and frees all resources for this engine instance
