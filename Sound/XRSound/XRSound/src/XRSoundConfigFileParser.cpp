...
bool XRSoundConfigFileParser::ParseModuleSoundConfig(const char *pUniqueModuleName)
{
    ...
    char cbuf[256];
    sprintf(cbuf, "[%s] Using configuration file: %s", pUniqueModuleName, GetConfigFilenames());
    WriteLog(cbuf);
...
bool XRSoundConfigFileParser::ParseVesselSoundConfig(VESSEL *pVessel)
{
    ...
    for (int i = 0; i < csSanitizedClassName.length(); i++)
    {
        if (s_csIllegalCharacters.find(csSanitizedClassName[i]) != std::string::npos)
            csSanitizedClassName[i]= '_';   // replace invalid character
    }

    m_csOverrideFilename = std::string("XRSound/XRSound-") + csSanitizedClassName + ".cfg";
    struct stat s;
    bOverrideFileExists = (stat(m_csOverrideFilename.c_str(), &s) == 0);
    ...
    char cbuf[256];
    sprintf(cbuf, "[%s][class %s] Using configuration file(s): %s", pVessel->GetName(), pVesselClassName, GetConfigFilenames());
    WriteLog(cbuf);
...
bool XRSoundConfigFileParser::ParseLine(const char *pSection, const char *pPropertyName, const char *pValue, const bool bParsingOverrideFile)
{
    ...
    if (SECTION_MATCHES("SYSTEM"))
    {
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "WARNING parsing file [%s]: cannot override [SYSTEM] settings via a vessel class override file.", GetCurrentFilename(bParsingOverrideFile));
            WriteLog(cbuf);
            return false;
        }
    ...
    else if (SECTION_MATCHES("VESSEL"))
    {
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "WARNING parsing file [%s]: [VESSEL] properties are inherently vessel class-specific and therefore have no meaning in the global .cfg file.", GetCurrentFilename(bParsingOverrideFile));
            WriteLog(cbuf);
            return false;
        }
    ...
    else if (SECTION_STARTSWITH("animation_"))
    {
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "WARNING parsing file [%s]: [animation_*] properties are inherently vessel class-specific and therefore have no meaning in the global .cfg file.", GetCurrentFilename(bParsingOverrideFile));
            WriteLog(cbuf);
            return false;
        }
        ...
        { 
            char cbuf[256];
            sprintf(cbuf, "ERROR: Invalid [animation] section name ('%s'): missing or invalid animation ID.  Format should be [animation_#]; e.g., [animation_7].",
                pSection);
            WriteLog(cbuf);
            return false;
        }
...
invalid_section:
    ...
    if (*pSection == 0) {
        WriteLog("Missing [section] line (e.g., '[GENERAL]')");
    } else {
        char cbuf[256];
        sprintf(cbuf, "Invalid [section] value: '%s'", pSection);
        WriteLog(cbuf);
    }
    return false;
...
invalid_property_name:
    // our base class will provide more details in the line following this one
    char cbuf[256];
    sprintf(cbuf, "Invalid property name: '%s' in section [%s]", pPropertyName, pSection);
    WriteLog(cbuf);
    return false;
}
...
bool XRSoundConfigFileParser::AddOrUpdateAnimationState(int animationID, const char *pName, const char *pValue)
{
    ...
    if (strcasecmp(pName, "OpenCloseSoundID") == 0)
    {
        int soundID = -1;
        SSCANF1("%d", &soundID);
        VALIDATE_INT(&soundID, 0, INT_MAX, -1);
        if (soundID >= -0)
...
#define PARSE_PLAYBACK_TYPE(pbt) else if (strcasecmp(pValue, TO_STR(pbt)) == 0) type = XRSound::PlaybackType::pbt;
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "ERROR: Invalid playback type: '%s'.  Valid playback types are InternalOnly, BothViewFar, BothViewMedium, BothViewClose, Radio, and Wind.", pName);
            WriteLog(cbuf);
            return false;
        }
    ...
    { 
        ...
        if (bIsLandingGear)
        {
            ...
            char cbuf[256];
            sprintf(cbuf, "XRSoundConfigFileParser: using landing gear animation ID %d", animationID);
            WriteLog(cbuf);
        }
    }
...
#define PARSE_STATE_TYPE(type) else if (strcasecmp(pName, TO_STR(type)) == 0) state = AnimationState::StateType::type;
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "ERROR: Invalid sound event type: '%s'.  Valid sound event types are Opening, Open, Moving, Closing, and Closed.", pName);
            WriteLog(cbuf);
            return false;
        }
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "ERROR: Sound file does not exist or is not readable: '%s' (remember that sound paths are relative to $ORBITER_ROOT)", pValue);
            WriteLog(cbuf);
            return false;
        }
...
const char *AnimationSounds::GetWavForAnimationState(AnimationState::StateType state) const
{
    ...
    if (it != m_soundStateWavFileMap.end())
    {
        ...
        if (soundID >= 0)
            pcsSoundPath = it->second.c_str();   // CString lives in our map, so it's OK to return a pointer to it
    }
