...
void AFCtrlModeDefaultSoundPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    ...
    if (afCtrlMode != m_previousAFCtrlMode)
    {
        ...
        case 0:
            PlaySwitch(false);  // "Off" click sound
            LoadAndPlayWavUsingID(XRSound::AFOff, GetConfig().AFOff.c_str(), false, XRSound::PlaybackType::Radio);
            break;

        case 1:
            PlaySwitch(true);  // "On" click sound
            LoadAndPlayWavUsingID(XRSound::AFPitch, GetConfig().AFPitch.c_str(), false, XRSound::PlaybackType::Radio);
            break;

        case 2:
            PlaySwitch(true);  // "On" click sound
            LoadAndPlayWavUsingID(XRSound::AFOn, GetConfig().AFOn.c_str(), false, XRSound::PlaybackType::Radio);
            break;
...
void LogThrusterDataPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    if (GetConfig().LogThrusterData && simt >= m_simtNextLog)
    {
        ...
        for (int i = 0; i < pVessel->GetThrusterCount(); i++)
        {
            ...
            if (thLevel > 0)
            {
                ...
                char cbuf[256];
                sprintf(cbuf, "LogThrusterData: [thruster index %u] thrust level = %lf, thrust = %lf kN", i, thLevel, thrust);
                WriteLog(cbuf);
            }
        }
    }
...
void CustomEnginesDefaultSoundPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    ...
    if (m_bFirstRun)
    {
        ...
        char cbuf[256];
        sprintf(cbuf, "CustomEnginesDefaultSoundPreStep: using custom engine sound '%s' for thrusters w/indexes [%s]", static_cast<const char*>(m_pEngine->GetWavFilename(soundID)), thrusterIdxStr.c_str());
        WriteLog(cbuf);
    }
...
void DisableAutopilotsForTimeAccPreStep::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    ...
    if (currentTimeACC > maxTimeAccForAP)
    {
        ...
        for (int i = 1; i <= 7; i++)
        {
            ...
            {
                char cbuf[256];
                sprintf(cbuf, "DisableAutopilotsForTimeAccPreStep: auto-disabling autopilot w/navmode %d due to time acceleration (%dx) [configured max time acc threshold for autopilots is %dx]",
