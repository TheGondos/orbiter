...
{
    char cbuf[256];;
    sprintf(cbuf, "XRSoundEngine::FreeResources: freeing XRSound engine resources for vessel '%s'",
        GetVesselName().c_str());
    s_globalConfig.WriteLog(cbuf);
    ...
    m_hVessel(hVessel)
{
    ...
    {
        char cbuf[256];
        sprintf(cbuf, "Error parsing configuration file(s) '%s' -- see above error messages for details.", m_pConfig->GetConfigFilenames());
...
void VesselXRSoundEngine::clbkPreStep(const double simt, const double simdt, const double mjd)
{
    if (IsSoundEngineInitialized())
    {
        ...
        for (vector<SoundPreStep *>::iterator it = m_allSoundPreSteps.begin(); it != m_allSoundPreSteps.end(); it++) {
            SoundPreStep *sps = *it;
            sps->clbkPreStep(simt, simdt, mjd);
        }
...
void VesselXRSoundEngine::PollAllAnimationStates()
{
    ...
    for (int animID = 0; animID < animationCount; animID++)
    {
        ...
        {
            char cbuf[256];
            sprintf(cbuf, "  >> LogVesselAnimations: [%s][animation ID = %d, state = %s]", GetVesselClassName().c_str(), animID, animationState.ToStr());
            WriteLog(cbuf);
        }
    }
}
