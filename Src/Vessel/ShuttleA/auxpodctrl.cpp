...
void set_needle_vtx (unsigned int which, double angle, NTVERTEX *vtx)
{
...
void set_preset_vtx (unsigned int which, double angle, NTVERTEX *vtx)
{
...
void AuxPodCtrl::AddMeshData2D (MESHHANDLE hMesh, int grpidx, int grpidx_disp)
{
	// Controls
...
bool AuxPodCtrl::ProcessMouse2D (int event, int mx, int my)
{
	...
	if (mode) {
		...
		if (mode == 1) pod_angle_ref = std::min (pod_angle_ref+dangle, PI);
		else           pod_angle_ref = std::max (pod_angle_ref-dangle, 0.0);
		for (i = 0; i < 2; i++)
			if ((ctrl >> i) & 1)
