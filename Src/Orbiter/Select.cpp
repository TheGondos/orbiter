...
Select::Select() : GUIElement("Select", "Select") {
    show = false;
    m_Opened = false;
    m_Title = "Selection";
    ...
    for (auto& e : entries) {
        if (e.m_Flags & ITEM_SEPARATOR) {
            ...
            if (e.m_SubEntries.size() == 0) {
                ...
                m_cbSubmenu(this, i, const_cast<char*>(e.m_Text.c_str()), m_UserData);
            }
            if (ImGui::BeginMenu(e.m_Text.c_str(), e.m_SubEntries.size() != 0)) {
                ...
                if (ImGui::IsItemHovered(ImGuiHoveredFlags_RectOnly) && !(e.m_Flags & ITEM_NOHILIGHT)) {
                    if (ImGui::IsMouseReleased(0)) {
                        m_cbEnter(this, i, const_cast<char*>(e.m_Text.c_str()), m_UserData);
                        show = false;
...
InputBox::InputBox() : GUIElement("InputBox", "InputBox") {
    show = false;
    m_Opened = false;
    m_Title = "InputBox";
}
