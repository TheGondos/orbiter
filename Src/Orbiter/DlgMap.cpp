...
DlgMap::DlgMap() : GUIElement("Map", "DlgMap") {
    show = false;
...
void DlgMap::DrawMap() {
    ...
    if (sz.x >= 1.0f && sz.y >= 1.0f) {
        ...
        //ImGui::Button("test", ImVec2(sz.x, sz.y));
        ...
        if (ImGui::IsItemActive()) {
            ...
            double scale = min(sz.x, 2.0f * sz.y);
            double scalefac = m_Map->ZoomFac() * scale / Pi2;

            double lng = lngc - io.MouseDelta.x / scalefac;
            double lat = max(-Pi05, min(Pi05, latc + io.MouseDelta.y / scalefac));
