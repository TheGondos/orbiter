...
const std::string DlgPlaybackEditor::etype = "DlgPlaybackEditor";
...
DlgPlaybackEditor::DlgPlaybackEditor(const std::string &name) : GUIElement(name, "DlgPlaybackEditor") {
    show = false;
	m_sysfname = nullptr;
...
void DlgPlaybackEditor::Show() {
	if(show) {
		ImGui::Begin("Playback Editor", &show);
        ...
        ImVec2 outer_size = ImVec2(0.0f, ImGui::GetContentRegionAvail().y - 50);
        if (ImGui::BeginTable("table_scrolly", 3, flags, outer_size))
        {
            ImGui::TableSetupScrollFreeze(0, 1); // Make top row always visible
            ImGui::TableSetupColumn("Timestamp", ImGuiTableColumnFlags_None);
            ImGui::TableSetupColumn("Event", ImGuiTableColumnFlags_None);
