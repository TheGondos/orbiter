...
const std::string DlgCamera::etype = "DlgCamera";

DlgCamera::DlgCamera(const std::string &name) : GUIElement(name, "DlgCamera") {
    show = false;
    m_SelectedPreset = -1;
}
...
void DlgCamera::DrawGround() {
    ...
    ImGui::BeginChild("ChildL", ImVec2(ImGui::GetContentRegionAvail().x * 0.5f, ImGui::GetContentRegionAvail().y), true, window_flags);
    {
        for (int i = 0; i < g_psys->nGrav(); i++) {
            ...
            if (planet->nGroundObserver() > 0 && ImGui::CollapsingHeader(planet->Name())) {
