...
Mesh::Mesh (NTVERTEX *vtx, int nvtx, uint16_t *idx, int nidx, int matidx, int texidx)
{
	nGrp = nMtrl = nTex = 0;
	GrpVis   = 0;
	GrpSetup = false;
...
void Mesh::Set (const Mesh &mesh)
{
	...
	if ((nGrp = mesh.nGrp)) {
		...
		for (i = 0; i < nGrp; i++) {
			...
			Grp[i].Idx = new uint16_t[Grp[i].nIdx]; TRACENEW
			memcpy (Grp[i].Idx, mesh.Grp[i].Idx, Grp[i].nIdx*sizeof(uint16_t)); 
		}
	}
	if ((nMtrl = mesh.nMtrl)) {
	...
	if ((GrpSetup = mesh.GrpSetup)) {
		GrpCnt = new glm::fvec3[nGrp]; TRACENEW
		memcpy (GrpCnt, mesh.GrpCnt, nGrp*sizeof(glm::fvec3));
		GrpRad = new float[nGrp]; TRACENEW
		memcpy (GrpRad, mesh.GrpRad, nGrp*sizeof(float));
		GrpVis = new int[nGrp]; TRACENEW
		memcpy (GrpVis, mesh.GrpVis, nGrp*sizeof(int));
	} else {
		GrpVis = 0;
	}
...
void Mesh::Setup ()
{
	...
	for (g = 0; g < nGrp; g++) {
		...
		if (Grp[g].MtrlIdx != SPEC_INHERIT && Grp[g].MtrlIdx >= (uint32_t)nMtrl) Grp[g].MtrlIdx = SPEC_DEFAULT;
		if (Grp[g].TexIdx != SPEC_INHERIT && Grp[g].TexIdx >= (uint32_t)nTex) Grp[g].TexIdx = SPEC_DEFAULT;
	}
}

void Mesh::SetupGroup (int grp)
{
	int i;
...
int Mesh::AddGroup (NTVERTEX *vtx, int nvtx, uint16_t *idx, int nidx,
	int mtrl_idx, int tex_idx, int16_t zbias, int flag, bool deepcopy)
{
	int n;
	GroupSpec *g, *tmp_Grp = new GroupSpec[nGrp+1]; TRACENEW
	glm::fvec3 *tmp_Cnt = new glm::fvec3[nGrp+1]; TRACENEW
	float *tmp_Rad = new float[nGrp+1]; TRACENEW
	int *tmp_Vis = new int[nGrp+1]; TRACENEW
	if (nGrp) {
		memcpy (tmp_Grp, Grp, nGrp*sizeof(GroupSpec));
		memcpy (tmp_Cnt, GrpCnt, nGrp*sizeof(glm::fvec3));
		memcpy (tmp_Rad, GrpRad, nGrp*sizeof(float));
		memcpy (tmp_Vis, GrpVis, nGrp*sizeof(int));
		delete []Grp;
		delete []GrpCnt;
		delete []GrpRad;
		delete []GrpVis;
	...
	if (deepcopy) {
		...
		g->Idx = new uint16_t[nidx];
		if (idx) memcpy (g->Idx, idx, nidx*sizeof(uint16_t));
		else     memset (g->Idx, 0, nidx*sizeof(uint16_t));
	} else {
		g->Vtx = vtx;
		g->Idx = idx;
	}
	...
	if (GrpSetup) {
		SetupGroup (nGrp);
		if (g->MtrlIdx != SPEC_INHERIT && g->MtrlIdx >= (uint32_t)nMtrl)
			g->MtrlIdx = SPEC_DEFAULT;
		if (g->TexIdx != SPEC_INHERIT && g->TexIdx >= (uint32_t)nTex)
			g->TexIdx = SPEC_DEFAULT;
	}
	return nGrp++;
}

bool Mesh::AddGroupBlock (int grp, const NTVERTEX *vtx, int nvtx, const uint16_t *idx, int nidx)
{
	if (grp >= nGrp) return false;

	GroupSpec *g = Grp+grp;
	uint16_t vofs = (uint16_t)g->nVtx;
	...
	uint16_t *i = new uint16_t[g->nIdx+nidx];
	if (g->nIdx) {
		memcpy (i, g->Idx, g->nIdx*sizeof(uint16_t));
		delete []g->Idx;
	}
	if (nidx) {
...
void Mesh::AddMesh (Mesh &mesh)
{
	for (int i = 0; i < mesh.nGroup(); i++) {
		...
		idx2 = new uint16_t[gs->nIdx];      memcpy (idx2, gs->Idx, gs->nIdx*sizeof(uint16_t)); TRACENEW
		AddGroup (vtx2, gs->nVtx, idx2, gs->nIdx);
	}
}

bool Mesh::DeleteGroup (int grp)
{
	if (grp < nGrp) {
		delete []Grp[grp].Vtx;
		delete []Grp[grp].Idx;
...
int Mesh::GetGroup (int grp, GROUPREQUESTSPEC *grs)
{
...
int Mesh::EditGroup (int grp, GROUPEDITSPEC *ges)
{
	if (grp >= nGrp) return 1;
	GroupSpec *g = Grp+grp;
	int i, vi;
...
bool Mesh::DeleteMaterial (int matidx)
{
	int i, j;
	...
	for (i = 0; i < nGrp; i++) {
		if (Grp[i].MtrlIdx >= (uint32_t)matidx) {
			if (Grp[i].MtrlIdx == (uint32_t)matidx) Grp[i].MtrlIdx = 0;
			else Grp[i].MtrlIdx--;
		}
	}
...
void Mesh::ScaleGroup (int grp, float sx, float sy, float sz)
{
...
void Mesh::TranslateGroup (int grp, float dx, float dy, float dz)
{
	int i, nv = Grp[grp].nVtx;
	NTVERTEX *vtx = Grp[grp].Vtx;
	for (i = 0; i < nv; i++) {
...
void Mesh::RotateGroup (int grp, RotAxis axis, float angle)
{
	int i, nv = Grp[grp].nVtx;
	NTVERTEX *vtx = Grp[grp].Vtx;
...
void Mesh::TransformGroup (int grp, const glm::fmat4 &mat)
{
	int i, nv = Grp[grp].nVtx;
	NTVERTEX *vtx = Grp[grp].Vtx;
	float x, y, z, w;
	...
	for (i = 0; i < nv; i++) {
		NTVERTEX &v = vtx[i];
		x = v.x*mat[0][0] + v.y*mat[1][0] + v.z* mat[2][0] + mat[3][0];
		y = v.x*mat[0][1] + v.y*mat[1][1] + v.z* mat[2][1] + mat[3][1];
		z = v.x*mat[0][2] + v.y*mat[1][2] + v.z* mat[2][2] + mat[3][2];
		w = v.x*mat[0][3] + v.y*mat[1][3] + v.z* mat[2][3] + mat[3][3];
    	v.x = x/w;
		v.y = y/w;
		v.z = z/w;

		x = v.nx*mat[0][0] + v.ny*mat[1][0] + v.nz* mat[2][0];
		y = v.nx*mat[0][1] + v.ny*mat[1][1] + v.nz* mat[2][1];
		z = v.nx*mat[0][2] + v.ny*mat[1][2] + v.nz* mat[2][2];
...
void Mesh::Transform (const glm::fmat4 &mat)
{
	for (int grp = 0; grp < nGrp; grp++)
		TransformGroup (grp, mat);
}

void Mesh::TexScaleGroup (int grp, float su, float sv)
{
	int i, nv = Grp[grp].nVtx;
	NTVERTEX *vtx = Grp[grp].Vtx;
	for (i = 0; i < nv; i++) {
...
void Mesh::CalcNormals (int grp, bool missingonly)
{
	const float eps = 1e-8f;
	int i, nv = Grp[grp].nVtx, nt = Grp[grp].nIdx/3;
	uint16_t *idx = Grp[grp].Idx;
	NTVERTEX *vtx = Grp[grp].Vtx;
	...
	for (i = 0; i < nt; i++) {
		...
		glm::fvec3 V01 = { vtx[i1].x - vtx[i0].x, vtx[i1].y - vtx[i0].y, vtx[i1].z - vtx[i0].z };
		glm::fvec3 V02 = { vtx[i2].x - vtx[i0].x, vtx[i2].y - vtx[i0].y, vtx[i2].z - vtx[i0].z };
		glm::fvec3 V12 = { vtx[i2].x - vtx[i1].x, vtx[i2].y - vtx[i1].y, vtx[i2].z - vtx[i1].z };
		glm::fvec3 nm = glm::cross(V01, V02);
		float len = nm.length();

		if (len >= eps) {
			nm.x /= len, nm.y /= len, nm.z /= len;
			float d01 = V01.length();
			float d02 = V02.length();
			float d12 = V12.length();
			if (calcNml[i0]) {
	...
	for (i = 0; i < nv; i++)
		if (calcNml[i]) {
			glm::fvec3 nm = { vtx[i].nx, vtx[i].ny, vtx[i].nz };
			float len = nm.length();
...
void Mesh::CalcTexCoords (int grp)
{
	...
	for (i = 0; i < nv; i++) {
		glm::fvec3 pos = {vtx[i].x, vtx[i].y, vtx[i].z};
...
bool Mesh::SetTexture (int texidx, SURFHANDLE tex, bool release_old)
{
...
void Mesh::SetTexMixture (int grp, int ntex, float mix)
{
	if (grp < nGrp && --ntex < MAXTEX && Grp[grp].TexIdxEx[ntex] != SPEC_DEFAULT)
		Grp[grp].TexMixEx[ntex] = mix;
}

void Mesh::SetTexMixture (int ntex, float mix)
{
	if (--ntex < MAXTEX) {
		for (int grp = 0; grp < nGrp; grp++)
...
/*
int Mesh::Render (LPDIRECT3DDEVICE7 dev)
{
	return 0;
}

void Mesh::RenderGroup (LPDIRECT3DDEVICE7 dev, int grp, bool setstate) const
{
...
{
	char cbuf[256];
	int i, j, g, ngrp, nvtx, ntri, nidx = 0, nmtrl, mtrl_idx, ntex, tex_idx, flag, res;
	int uflag;
	int16_t zbias;
	MATERIAL mtrl;
	...
	for (g = 0, term = false; g < ngrp && !term; g++) {
		...
		for (;;) {
			...
			if (!_strnicmp (cbuf, "MATERIAL", 8)) {       // read material index
				...
				idx = new uint16_t[nidx]; TRACENEW
				...
				if (flipidx)
					for (i = 0; i < ntri; i++) {
						uint16_t tmp = idx[i*3+1]; idx[i*3+1] = idx[i*3+2]; idx[i*3+2] = tmp;
					}
...
void CreateSpherePatch (Mesh &mesh, int nlng, int nlat, int ilat, int res, int bseg, bool reduce, bool outside)
{
	...
	for (i = n = 0; i <= res; i++) {  // loop over longitudinal strips
		...
		for (j = 0; j <= nseg; j++) {
			...
			Vtx[n].x = Vtx[n].nx = clat*clng;
			Vtx[n].y = Vtx[n].ny = slat;
			Vtx[n].z = Vtx[n].nz = clat*slng;
			Vtx[n].tu = nseg ? (c1*j)/nseg+c2 : 0.5; // overlap to avoid seams
			Vtx[n].tv = (c1*(res-i))/res+c2;
