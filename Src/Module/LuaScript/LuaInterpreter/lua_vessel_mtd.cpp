...
int Interpreter::v_get_status (lua_State *L)
{
	...
	int version = (lua_gettop(L) >= 2) ? lua_tointeger(L, 2) : 2; // default to version 2
...
int Interpreter::v_edit_airfoil (lua_State *L)
{
	...
	int flag = luamtd_tointeger_safe(L, 3, funcname);
...
int Interpreter::v_create_controlsurface (lua_State *L)
{
	...
	if (lua_gettop(L) >= 6) {
		...
		if (lua_gettop(L) >= 7) {
			...
			if (lua_gettop(L) >= 8) {
				anim = (unsigned int)luamtd_tointeger_safe(L, 8, funcname);
			}
		}
	}
...
int Interpreter::v_dockingstatus (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 2, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	unsigned int port = luamtd_tointeger_safe(L, 2, funcname);
...
int Interpreter::v_undock (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 2, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	unsigned int idx = (unsigned int)luamtd_tointeger_safe(L, 2, funcname);
	v->Undock (idx);
	return 0;
}
...
int Interpreter::v_set_transponderchannel (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 2, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	int ch = (int)luamtd_tointeger_safe(L, 2, funcname);
...
int Interpreter::v_set_idschannel (lua_State *L)
{
	...
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	DOCKHANDLE hDock = (DOCKHANDLE)luamtd_tolightuserdata_safe(L, 2, funcname);
	int ch = (int)luamtd_tointeger_safe(L, 3, funcname);
...
int Interpreter::v_init_navradios (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 2, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	int nnav = (int)luamtd_tointeger_safe(L, 2, funcname);
...
int Interpreter::v_set_navchannel (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 3, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	int n = (int)lua_tointeger_safe(L, 2, funcname);
	int ch = (int)lua_tointeger_safe(L, 3, funcname);
	ASSERT_SYNTAX(ch < 640, "Argument 2: out of range");
...
int Interpreter::v_get_lightemitter (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 2, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	int idx = (int)luamtd_tointeger_safe(L, 2, funcname);
...
int Interpreter::v_insert_mesh (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 3, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	unsigned int midx, idx = (unsigned int)lua_tointeger_safe(L, 3, funcname);
	VECTOR3 ofs, *pofs = 0;
	if (lua_gettop(L) >= 4) {
...
int Interpreter::v_get_meshcount (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 1, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	unsigned int count = v->GetMeshCount();
	lua_pushnumber (L, count);
	return 1;
}
...
int Interpreter::v_create_animation (lua_State *L)
{
	...
	unsigned int anim = v->CreateAnimation (istate);
...
int Interpreter::v_del_animation (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 2, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	unsigned int anim = (unsigned int)luamtd_tointeger_safe(L, 2, funcname);
...
int Interpreter::v_get_animation (lua_State *L)
{
	...
	AssertMtdMinPrmCount(L, 2, funcname);
	VESSEL *v = lua_tovessel_safe(L, 1, funcname);
	unsigned int anim = luamtd_tointeger_safe(L, 2, funcname);
...
int Interpreter::v_add_exhauststream (lua_State *L)
{
	...
	pss.flags = (lua_isnumber(L,-1) ? (int)(lua_tonumber(L,-1)+0.5) : 0);
	lua_pop(L,1);
