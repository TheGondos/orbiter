    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
				...
				if (pcmd->GetTexID() == bd->FontTexture) {
					texture = bd->FontTexture;
				}
				else {
					texture = SURFACE(pcmd->GetTexID())->GetTexture();
				}

				HRESULT res;
				res = bd->pd3dDevice->SetTexture(0, texture);
				assert(res == S_OK);
				res = bd->pd3dDevice->SetScissorRect(&r);
				assert(res == S_OK);
				res = bd->pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, pcmd->VtxOffset + global_vtx_offset, 0, (UINT)cmd_list->VtxBuffer.Size, pcmd->IdxOffset + global_idx_offset, pcmd->ElemCount / 3);
				assert(res == S_OK);
				if (pcmd->GetTexID() != bd->FontTexture) {
					g_client->clbkReleaseSurface(SURFACE(pcmd->GetTexID()));
				}
            }
        }
